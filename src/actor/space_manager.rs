...
//! The SpaceManager owns space/screen enablement state and sits between the
//! WindowServer and the Reactor on the reactor thread. It receives screen and
//! space events from WindowServer, toggle/command events from WmController,
//! and forwards filtered reactor events downstream.
...
impl SpaceManager {
    ...
    fn on_event(&mut self, event: Event) {
        match event {
            ...
            Event::ConfigUpdated(config) => {
                self.send_space_enabled_status();
                self.reactor_tx.send(reactor::Event::ConfigChanged(config.clone()));
                self.config = config;
            }
        }
    }
    ...
    fn update_hotkey_state(&mut self) {
        ...
        if active != self.hotkeys_active {
            ...
            debug!(hotkeys_active = active, "Updating hotkey state");
            _ = self.wm_tx.send((tracing::Span::current(), WmEvent::HotkeysActive(active)));
        }
    }
}
