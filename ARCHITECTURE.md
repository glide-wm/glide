...
The Reactor produces a cleaned up stream of events for the LayoutManager, which returns desired window frames (position and size), raises, and UI state. The reactor turns these into requests for the App threads app threads (set window frame, raise, begin/end animation), the RaiseManager, GroupBars, and Status actors.
